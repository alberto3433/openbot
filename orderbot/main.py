"""
Orderbot API - Main Application Entry Point
=================================================

This is the main FastAPI application for the Orderbot ordering system.
It orchestrates all components by configuring the application, middleware,
and routing.

Application Overview:
---------------------
The Orderbot is an AI-powered ordering system that handles:
...
-------------------
    ...
    # Production
    gunicorn orderbot.main:app -w 4 -k uvicorn.workers.UvicornWorker
"""
...
async def lifespan(app: FastAPI):
    ...
    # Shutdown
    logger.info("Orderbot API shutting down")
...
app = FastAPI(
    title="Orderbot API",
    description="API for the Orderbot ordering system",
    version=__version__,
    openapi_tags=[
...
async def _orderbot_error_handler(request: Request, exc: OrderBotError):
...
app.add_exception_handler(OrderBotError, _orderbot_error_handler)
...
api_v1_router.include_router(admin_ingredient_subcategories_router)
