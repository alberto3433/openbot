...
# Import sub-handler classes AFTER RESPONSES is defined to avoid circular import issues
from .order_history_data import OrderHistoryData
from .order_history_reorder import OrderHistoryReorder
...
class OrderHistoryHandler(ContextMixin, MenuDataMixin):
    ...
    def __init__(
        ...
        # Sub-handlers
        self._data = OrderHistoryData(self)
        self._reorder = OrderHistoryReorder(self)
    ...
    def handle_reorder_specific_item(
        ...
        return self._reorder.handle_reorder_specific_item(item_ref, order)
    ...
    def handle_reorder_with_modifications(
        ...
        return self._reorder.handle_reorder_with_modifications(modification_text, order)
    ...
    def handle_reorder_item_selection(
        ...
        return self._reorder.handle_reorder_item_selection(user_input, order)
    ...
    def handle_reorder_offer_response(
        ...
        return self._reorder.handle_reorder_offer_response(user_input, order)

    def _get_customer_phone(self, order: OrderTask) -> str | None:
        return self._data._get_customer_phone(order)
    ...
    def _get_order_history(
        ...
        return self._data._get_order_history(phone, days=days, limit=limit)

    def _format_order_date(self, date_str: str | None) -> str:
        return self._data._format_order_date(date_str)

    def _format_items_for_display(self, items: list) -> str:
        return self._data._format_items_for_display(items)

    def _validate_items(self, items: list) -> tuple[list, list]:
        return self._data._validate_items(items)
    ...
    def apply_modifications(
        ...
        return self._data.apply_modifications(items, modification_text)
    ...
    def _add_item_from_history(
        ...
        return self._reorder._add_item_from_history(item_data, order, silent=silent)
